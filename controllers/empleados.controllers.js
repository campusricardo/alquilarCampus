...
const postReservas = async (req, res) => {
  try {
    ...
    const createReserva = new Reserva({
      ID_Cliente,
      ID_Automovil,
...
const deleteReserva = async (req, res) => {
  try {
    ...
    const deleteReserva = await Reserva.findByIdAndUpdate(id, {
      Estado: false,
    });
...
const postAliquiler = async (req, res) => {
  try {
    const { ID_Cliente, ID_Automovil, Fecha_Inicio, Fecha_Fin, Costo_Total } =
      req.body;

    const createAlquiler = new Alquiler({
      ID_Cliente,
      ID_Automovil,
      Fecha_Inicio: new Date(Fecha_Inicio),
      Fecha_Fin: new Date(Fecha_Fin),
...
const deleteAlquiler = async (req, res) => {
  try {
    ...
    const deleteReserva = await Alquiler.findByIdAndUpdate(id, {
      Estado: false,
    });
...
const postDevolucion = async (req, res) => {
  try {
    ...
    const newDevolucion = new RegistroDevolucion({
      ID_Alquiler,
      ID_Empleado,
...
const postEntrega = async (req, res) => {
  try {
    ...
    const newEntrega = new RegistroEntrega({
      ID_Alquiler,
      ID_Empleado,
...
const empleadosVendedores = async (req, res) => {
  const vendedores = await Empleado.find({ Cargo: "Vendedor" });
  res.json({
